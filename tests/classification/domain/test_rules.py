...
class RuleClassifierTests(unittest.TestCase):
    ...
    def test_conflict_marks_ambiguous_and_keeps_best(self):
    ...
    def test_stage_update_conflict_keeps_stage_and_marks_ambiguous(self):
        page = _page(
            "1 + 2 = Tor",
            ("Category:Sub-chapter 106 Stages", "Category:Zero Legends Stages"),
            content="Like other stages added in Version 13.0.",
        )
        result = RuleBasedClassifier().classify(page)
        self.assertEqual(result.entity_type, "stage")
        self.assertTrue(result.is_ambiguous)
        self.assertIn("low_margin_conflict:stage_vs_update", result.reasons)
